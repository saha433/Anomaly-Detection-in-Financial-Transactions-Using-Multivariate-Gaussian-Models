...
st.caption("Mathematical Multivariate Gaussian PDF | financial_fraud_detection_dataset")

with st.sidebar:
    st.header("Settings")
    data_source = st.radio("Dataset source", ["Local CSV path", "Upload CSV/XLSX"])
    model_choice = st.radio("Detection model", ["Multivariate Gaussian", "Isolation Forest", "Both"])
    has_labels = st.checkbox("Dataset has a fraud label column", value=True)
    max_rows = st.number_input(
        ...
        value=200_000,
        step=10_000,
        help="Use a sample for fast Streamlit runs; increase for final evaluation."
    )
...
if data_source == "Upload CSV/XLSX":
    uploaded_file = st.file_uploader(
        "Upload financial_fraud_detection_dataset.csv or another transaction file",
        type=["csv", "xlsx"],
        help="Large files may exceed Streamlit's upload limit; use Local CSV path for the 759 MB dataset."
    )
    if uploaded_file is None:
        st.info("Upload a CSV/XLSX to get started.")
        st.stop()
else:
    local_path = st.text_input("Local dataset path", value=DEFAULT_DATA)
    if not os.path.exists(local_path):
        st.error(f"File not found: {local_path}")
        st.stop()

@st.cache_data(show_spinner="Loading data...")
def load_df(file, path, nrows):
    if file is not None:
...
df = load_df(uploaded_file, local_path, int(max_rows))
...
with st.spinner("Training model..."):
    ...
    if model_choice in ["Multivariate Gaussian", "Both"]:
    ...
    if model_choice in ["Isolation Forest", "Both"]:
        iso_contamination = float(y_for_train.mean()) if y_for_train is not None else contamination
        iso = IsolationForestDetector(contamination=max(iso_contamination, 1e-5))
        iso.fit(X_normal)
...
elif iso_preds is not None:
    flagged = result_df[result_df["iso_anomaly"] == 1].sort_values("iso_risk_score", ascending=False)
else:
    flagged = result_df.head(0)
...
if mvg is not None:
    ...
    st.caption("Computed from the explicit multivariate Gaussian PDF")

    fig, ax = plt.subplots(figsize=(10, 4))
    log_p = mvg.log_score(X_scaled)
    log_eps = mvg.log_epsilon
...
if y is not None and y.sum() > 0:
    ...
    model_outputs = []
    if mvg_preds is not None:
        model_outputs.append((mc1, "Multivariate Gaussian", mvg_preds))
    if iso_preds is not None:
        model_outputs.append((mc2, "Isolation Forest", iso_preds))

    for col_ui, name, preds in model_outputs:
        with col_ui:
            st.markdown(f"**{name}**")
